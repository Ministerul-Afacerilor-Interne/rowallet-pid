...
*Why multiple approved channels are necessary.* Restricting enrolment solely to holders of a CEI with NFC would exclude the majority of Romanian citizens holding traditional identity cards (model 1997), simple identity cards (CIS), or residing abroad. Under [CIR 2015/1502] Section 2.1.2, in-person identity proofing at a counter by an authorized officer directly satisfies the requirement for level of assurance **high** when verified against authoritative national registers (RNEP). Channel A provides this universal in-person onboarding path, while Channel B allows an optimized hybrid flow for holders of electronic tokens (CEI or ICAO Doc 9303 compliant Romanian biometric passports).

*What the two scans are for, and why they are not ceremony.* The identity checks at the counter establish who the User is. They do not establish **which device receives the attestation**, and without that a PID could be issued to a Wallet Unit other than the one that initiated the request — the failure mode activation exists to prevent. The first scan binds the counter session to the Wallet Unit; the second, over a code that lives for less than a minute, records the User's authorisation to issue while they are still present. That is why RO_PID_32 identifies these steps as the activation process rather than adding a separate one.

*On the assurance level, and the role of identity documents.* The Romanian electronic identification scheme is not notified at level of assurance high, and nothing in this Rulebook requires that it be. The level of assurance in RO_PID_02 rests on the identity proofing — the comparison of the person present against their official identity document and against the RNEP record at the counter — which is what [CIR 2015/1502] Section 2.1.2 asks for and which does not depend on prior electronic scheme notification. For Channel B, electronic token verification (validating the SOD against the national CSCA) provides additional cryptographic assurance of document possession.
